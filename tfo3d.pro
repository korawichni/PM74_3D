...
Group{
  DefineGroup[
	Primary, Secondary0, Secondary1, Surf_elec, DomainC, DomainCC, Domain
  ];
  ...
  In_current = Region[{Primary_In,Secondary0_In,Secondary1_In}];
  Out_current = Region[{Primary_Out,Secondary0_Out,Secondary1_Out}];
...
Function{
  ...
  DefineFunction[
    mu, nu, sigma, rho, js, Irms, radius, vDir
  ] ;
...
Group {
  Surf_a_NoGauge = Region [ {Surf_bn0} ] ;
}
  ...
  { Name Hregion_u_3D ; Type Form0 ;
    BasisFunction {
      { Name sr ; NameOfCoef ur ; Function BF_GroupOfNodes ;
        Support DomainC ; Entity GroupsOfNodesOf[ In_current ] ; }
    }
    GlobalQuantity {
...
Resolution {
/*   { Name TangentialVector ;
    System {
      { Name SysVector ; NameOfFormulation FindVector ;}
    }
    Operation {
      CreateDir["res/"];

      Generate[SysVector] ; // Solve[Sys] ;
      // SaveSolution[Sys] ;
